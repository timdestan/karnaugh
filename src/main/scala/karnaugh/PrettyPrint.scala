...
object Tree {
  ...
  case class And(children: List[Tree], ctx: Context) extends Tree {
    def renderedChildren = children.mkString(" ∧ ")

    override def toString = ctx match {
      case CNot | COr => wrapInParens(renderedChildren)
      case CTop | CAnd => renderedChildren
    }
  }

  case class Or(children: List[Tree], ctx: Context) extends Tree {
    def renderedChildren = children.mkString(" ∨ ")
  ...
  def apply(exp: Exp): Tree = {
    def loop(exp: Exp, ctx: Context): Tree = exp match {
      case Exp.Variable(v) => Atom(v)
      case Exp.Not(e) => Not(loop(e, CNot))
      case Exp.Or(es) => Or(es.map(loop(_, COr)), ctx)
      case Exp.And(es) => And(es.map(loop(_, CAnd)), ctx)
